...
use driver_virtio::{self, BufferDirection, PhysAddr, VirtIoHal};
...
cfg_if! {
    if #[cfg(bus = "pci")] {
        ...
        type E1000Transport = driver_virtio::PciTransport;
    } else if #[cfg(bus =  "mmio")] {
        type E1000Transport = driver_virtio::MmioTransport;
    }
}

/// A trait for VirtIO device meta information.
pub trait E1000DevMeta {
    const DEVICE_TYPE: DeviceType;

    type Device: BaseDriverOps;
    type Driver = E1000Driver<Self>;

    fn try_new(transport: E1000Transport) -> DevResult<AxDeviceEnum>;
}
...
cfg_if! {
    if #[cfg(net_dev = "e1000")] {
        ...
        impl E1000DevMeta for E1000Net {
            const DEVICE_TYPE: DeviceType = DeviceType::Net;
            type Device = driver_virtio::VirtIoNetDev<E1000HalImpl, E1000Transport, 64>;
...
pub struct E1000Driver<D: E1000DevMeta + ?Sized>(PhantomData<D>);

impl<D: E1000DevMeta> DriverProbe for E1000Driver<D> {
    #[cfg(bus = "mmio")]
    ...
    fn probe_pci(
        ...
        if let Some((ty, transport)) =
            driver_virtio::probe_pci_device::<E1000HalImpl>(root, bdf, dev_info)
        {
            if ty == D::DEVICE_TYPE {
                match D::try_new(transport) {
                    Ok(dev) => return Some(dev),
...
unsafe impl VirtIoHal for E1000HalImpl {
